...
public class TownGraphManager implements TownGraphManagerInterface {
	...
	public void populateTownGraph(File selectedFile) throws FileNotFoundException, IOException {
		...
		while (scanner.hasNextLine()) {
			...
			if (parts.length == 4) {
				// trailing and leading spaces are cringe :}
				for (String s : parts) {
					s.trim();
				}
				...
				this.getTownGraph().addEdge(start, end, distance, roadName);
	...
	public boolean addRoad(String town1, String town2, int weight, String roadName) {
		...
		return (this.getTownGraph().addEdge(t1, t2, weight, roadName) != null);
	}

	@Override
	public String getRoad(String town1, String town2) {
		return this.getTownGraph().getEdge(this.getTown(town1), this.getTown(town2)).getDescription();
	}

	@Override
	public boolean addTown(String v) {
		return this.getTownGraph().addVertex(new Town(v));
	}

	@Override
	public boolean containsTown(String v) {
		return this.getTownGraph().containsVertex(this.getTown(v));
	}

	@Override
	public boolean containsRoadConnection(String town1, String town2) {
		return this.getTownGraph().containsEdge(this.getTown(town1), this.getTown(town2));
	}

	@Override
	public ArrayList<String> allRoads() {
		ArrayList<String> res = new ArrayList<String>();
		for (Road r : this.getTownGraph().getRoads()) {
	...
	public boolean deleteRoadConnection(String town1, String town2, String road) {
		return (this.getTownGraph().removeEdge(this.getTown(town1), this.getTown(town2), 1, road) != null);
	}

	@Override
	public boolean deleteTown(String v) {
		return (this.getTownGraph().removeVertex(this.getTown(v)));
	}

	@Override
	public ArrayList<String> allTowns() {
		ArrayList<String> res = new ArrayList<String>();
		for (Town r : this.getTownGraph().getTowns()) {
			res.add(r.getName());
		}
		Collections.sort(res);
		return res;
	...
	public ArrayList<String> getPath(String town1, String town2) {
		...
		return ((start == null || end == null) ? new ArrayList<String>()
				: this.getTownGraph().shortestPath(this.getTown(town1), this.getTown(town2)));
	}
	...
	public Town getTown(String string) {
		for (Town t : this.getTownGraph().getTowns()) {
	...
	/**
	 * @return the townGraph
	 */
	private Graph getTownGraph() {
		return this.townGraph;
	}
}
